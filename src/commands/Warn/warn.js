...
module.exports = {
  ...
  async execute(interaction, client) {
    if (interaction.member.roles.cache.has(process.env.MOD_ROLE_ID)) {
        ...
        interaction.guild.channels.cache.get(process.env.BOT_LOGGING).send({embeds:[warningEmbed]})

        await user.send({content: `:warning: You have been warned!\nReason: ${reason}`}).catch(console.log)
