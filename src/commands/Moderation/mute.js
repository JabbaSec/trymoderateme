...
const ms = require('ms');
  ...
  module.exports = {
    data: new SlashCommandBuilder()
      ...
      option
        .setName("duration")
        .setDescription("The length of time that the user will be muted for (1d/ 1h/ 1m).")
        .setRequired(true)
    )
      .addStringOption((option) =>
        option
      ...
      if (interaction.member.roles.cache.has(process.env.MOD_ROLE_ID)) {
        ...
        const muteEmbed = new EmbedBuilder()
          ...
          var timerId = setTimeout(async () => {
            member.roles.remove(mutedRole).catch((err) => interaction.guild.channels.cache
            .get(process.env.BOT_LOGGING)
            .send({ embeds: [leftEmbed] })
            .catch((err) => console.log("[MUTE] Error with sending the left embed.")))
