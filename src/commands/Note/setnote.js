...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('note').setDescription('Note details.').setRequired(true)),
  async execute(interaction, client) {
    if (interaction.member.roles.cache.has(process.env.MOD_ROLE_ID)) {
        ...
        interaction.guild.channels.cache.get(process.env.BOT_LOGGING).send({embeds:[noteEmbed]})
